...
\paragraph{Antithrombin III} % (fold)
...
\paragraph{Arterieller Thrombus (weißer Thrombus)} % (fold)
...
\paragraph{Venöser Thrombus (roter Thrombus)} % (fold)
...
\paragraph{Unfraktioniertes Heparin} % (fold)
...
\paragraph{Niedermolekulares Heparin (z.B. Enoxaparin, Nadroparin, Dalteparin)} % (fold)
...
\paragraph{Synthetische Pentasaccharide (z.B. Fondaparinux)} % (fold)
...
\paragraph{niedermolekulare Thrombin-Inhibitoren} % (fold)
\label{par:niedermolekulare_thrombin_inhibitoren}
\subparagraph{Argatroban} % (fold)
...
\subparagraph{Dagibatranetexilat} % (fold)
...
\end{itemize}
\paragraph{wenn Troponin-Test positiv, aber keine ST-Streckenhebung zusätzlich} % (fold)
...
\paragraph{bei eingetretenem Myokardinfarkt zusätzlich} % (fold)
...
\paragraph{Antiphlogistische Wirkung} % (fold)
...
\subparagraph{Lokale Reaktion} % (fold)
...
\subparagraph{Systemische Reaktion} % (fold)
\label{subp:subparagraph_name}
...
\paragraph{Analgetische Wirkung} % (fold)
...
\paragraph{Antipyretische Wirkung} % (fold)
...
\paragraph{Gastrointestinal (v.a. COX-1)} % (fold)
...
\paragraph{Renal  (COX-1 / COX-2)} % (fold)
...
\paragraph{ Provokation von asthmatischen Beschwerden bei Asthmatikern} % (fold)
...
\paragraph{erhöhtes Risiko für kardiovaskuläre Ereignisse} % (fold)
...
Acetylsalicylsäure\\
\end{tabularx}
\paragraph{Einsatz und Dosierung} % (fold)
...
% paragraph einsatz (end)
\paragraph{TNF$\alpha$/IL-1-Hemmstoffe} % (fold)
...
\paragraph{Entzündungshemmung durch Glukokortikoide} % (fold)
...
\paragraph{Immunsuppression} % (fold)
...
\paragraph{Pharmakokinetik von Glukokortikoiden} % (fold)
...
\paragraph{Dosierung / Applikation von Glukokortikoiden} % (fold)
...
\paragraph{Therapeutische Anwendung von Glukokortikoiden} % (fold)
\label{par:therapeutische_anwendung_von_glukokortikoiden}
\subparagraph{Substitutionstherapie} % (fold)
...
\subparagraph{„pharmakodynamische“ Therapie} % (fold)
...
\paragraph{Chronifizierung des Schmerzesbei pathologischen Zuständen: Periphere Sensibilisierung } % (fold)
...
\paragraph{Transmitter exzitatorischer nozizeptiver A$\delta$- un C-Fasern} % (fold)
...
\paragraph{Chronifizierung des Schmerzes bei pathologischen Zuständen:
...
\paragraph{Periaquäduktales Grau} % (fold)
...
\end{tabularx} 
Wirkung v.a. durch Hemmung der Cyclooxygenase (COX-1 und COX-2)	 
\paragraph{erwünschte Wirkqualitäten} % (fold)
\label{par:erw_nschte_wirkqualit_tten}
\subparagraph{analgetisch} % (fold)
...
\paragraph{antiphlogistisch /  antipyretisch} % (fold)
...
\end{tabularx}
\paragraph{Opioid-Rezeptoren} % (fold)
...
\end{tabularx}
% paragraph kontraindikationen (end)
\paragraph{wichtige unerwünschte Wirkungen bei Dauerschmerztherapie} % (fold)
\mbox{} \\
...
\paragraph{Opiatintoxikation} % (fold)
...
\paragraph{Weitere reine Agonisten} % (fold)
\label{subp:weitere_reine_agonisten}
(schwach wirksame Opioide der WHO Stude 2)
\subparagraph{Tilidin und Naloxon} % (fold)
...
\subparagraph{Levomethadon, Methadon} % (fold)
...
\paragraph{Partielle Agonisten} % (fold)
\label{par:partielle_agonisten}
\subparagraph{Buprenorphin} % (fold)
...
\paragraph{$\mu$-Opioid Agonisten mit hemmender Wirkung auf NA/5-HT-Wiederaufnahme} % (fold)
\label{par:_opioid_agonisten_mit_hemmender_wirkung_auf_na_5_ht_wiederaufnahme}
\subparagraph{Tramadol} % (fold)
\label{subp:tramadol}
...
\paragraph{Antagonisten} % (fold)
\label{par:antagonisten}
\subparagraph{Naloxon} % (fold)
\label{subp:naloxon}
...
\subparagraph{Methylnaltrexon} % (fold)
...
\paragraph{Körperliche Abhängigkeit} % (fold)
...
\paragraph{Psychische Abhängigkeit} % (fold)
...
\subparagraph{Reward-Systeme} % (fold)
...
\paragraph{Stufe 1 - Nicht-opioide Analgetika} % (fold)
\label{par:stufe_}
\mbox{} \\
\begin{tabularx}{\textwidth}{XXXX}
...
\paragraph{Stufe 2 - Mittelstarke Opiate/Opioide + ggf. nicht-opioide Analgetika} % (fold)
\label{par:stufe_}
\mbox{} \\
\begin
...
\end{tabularx}
% paragraph stufe_ (end)
\paragraph{Stufe 3 - Starke Opiate/Opioide + ggf. nicht-opioide Analgetika} % (fold)
\label{par:stufe_}
\mbox{} \\
\begin{tabularx}{\textwidth}{XXXX}
...
\end{tabularx}
\paragraph{Stufe 4 - Starke Opioide kontinuierlich i.v., s.c., peridural} % (fold)
\label{par:stufe_}
...
\label{sec:_strogene}
\paragraph{Natürliche Östrogene; geringe Bioverfügbarkeit}
\begin{tabularx}{\textwidth}{XXX}
Östradiol&Östriol&Östron\\ 
\end{tabularx}
\paragraph{Synthetische Östrogene}
\begin{tabularx}{\textwidth}{XXX}
...
\paragraph{unerw. Wirkungen} Blutungen, schmerzhafte Uteruskontraktionen, Übelkeit, Erbrechen, Durchfall, Kopfschmerzen
...
\label{sub:konzepte}
\paragraph{Einstufen-Kombinationspräparat} % (fold)
...
\paragraph{Depot-Gestagene} % (fold)
...
\paragraph{„postkoitale Kontrazeption“} % (fold)
...
\end{itemize}
\paragraph{Gründe für „Pillenversager“} % (fold)
...
\paragraph{Cyproteronacetat} % (fold)
...
\paragraph{Thyroxin (T$_4$)} % (fold)
\label{par:thyroxin_t__4_}
Prohormon
\paragraph{Trijodthyronin (T$_3$)} % (fold)
...
\end{tabularx}
% paragraph kontraindikationen (end)
\paragraph{Wechselwirkungen} % (fold)
...
\paragraph{Nebenwirkungen der Zytostatikatherapie} % (fold)
